	...
	cfft4(){
		...
		do{
			...
			for(int j=0;j<k;j+=2){
				...
				W[i++]=complex<T>(cos(2*(j+1)*t*pi/n),-sin(2*(j+1)*t*pi/n));
				W[i++]=W[i-2]*W[i-2];
				W[i++]=W[i-2]*W[i-2];
				W[i++]=W[i-3]*W[i-4];
				W[i++]=W[i-4]*W[i-4];
	...
	*/
	...
	void operator()(complex<T> *in, complex<T> *out)
	{
		...
		do
		{
			...
			r4 = out[i + 3].real();
			i4 = out[i + 3].imag();
			out[i].real(r1 + r2 + r3 + r4);
			out[i].imag(i1 + i2 + i3 + i4);
			out[i + 1].real(r1 + i2 - r3 - i4);
			out[i + 1].imag(i1 - r2 - i3 + r4);
			out[i + 2].real(r1 - r2 + r3 - r4);
			out[i + 2].imag(i1 - i2 + i3 - i4);
			out[i + 3].real(r1 - i2 - r3 + i4);
			out[i + 3].imag(i1 + r2 - i3 - r4);
			i += 4;
		} while (i < n);
		tt >>= 2;
		td <<= 2;
		do
		{
			...
			do
			{
				...
				r3 = out[i + td + td].real();
				i3 = out[i + td + td].imag();
				r4 = out[i + td + td + td].real();
				i4 = out[i + td + td + td].imag();
				out[i].real(r1 + r2 + r3 + r4);
				out[i].imag(i1 + i2 + i3 + i4);
				out[i + td].real(r1 + i2 - r3 - i4);
				out[i + td].imag(i1 - r2 - i3 + r4);
				out[i + td + td].real(r1 - r2 + r3 - r4);
				out[i + td + td].imag(i1 - i2 + i3 - i4);
				out[i + td + td + td].real(r1 - i2 - r3 + i4);
				out[i + td + td + td].imag(i1 + r2 - i3 - r4);
				i++;
				j++;
				for (; j < td; j++)
				{
					...
					r3 = out[i + td + td].real() * W[j * tt + j * tt].real() - out[i + td + td].imag() * W[j * tt + j * tt].imag();
					i3 = out[i + td + td].real() * W[j * tt + j * tt].imag() + out[i + td + td].imag() * W[j * tt + j * tt].real();
					r4 = out[i + td + td + td].real() * W[j * tt + j * tt + j * tt].real() - out[i + td + td + td].imag() * W[j * tt + j * tt + j * tt].imag();
					i4 = out[i + td + td + td].real() * W[j * tt + j * tt + j * tt].imag() + out[i + td + td + td].imag() * W[j * tt + j * tt + j * tt].real();
					out[i].real(r1 + r2 + r3 + r4);
					out[i].imag(i1 + i2 + i3 + i4);
					out[i + td].real(r1 + i2 - r3 - i4);
					out[i + td].imag(i1 - r2 - i3 + r4);
